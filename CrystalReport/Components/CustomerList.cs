...
namespace CrystalReport.Components
{
    public partial class CustomerList : UserControl
    {
        ...
        private void DisplayRecords(int startIndex, int count)
        {
            ...
            {
                dataGridView1.Rows.Add(fetchedData[i].id, fetchedData[i].cust_name, fetchedData[i].cust_phone, fetchedData[i].pcity,fetchedData[i].pstate,fetchedData[i].pstate,fetchedData[i].Balance);
            }
        }
        ...
        private async Task LoadData(string searchTerm)
        {
            ...
            {
                dataGridView1.Rows.Add(item.id, item.cust_name, item.cust_phone, item.pcity,item.pstate,item.Balance);
            }
